class Pastit < Formula
  ...
  def install
    (bin/"pastit").write <<~SH
      ...
      # Use a temporary file to safely handle massive streams without memory bottlenecks
      tmpfile=$(mktemp)
      trap 'rm -f "$tmpfile"' EXIT
      cat > "$tmpfile"

      response=$(curl -s -w "\\n%{http_code}" -X POST https://akinus21.com \
      ...
      printf "%s" "$body" | jq -r '"https://akinus21.com" + .key'
    SH
    chmod "+x", bin/"pastit"
  end 
